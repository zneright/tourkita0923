        ...
        const fetchData = async () => {
            try {
                ...
                if (selectedCategory === "Events") {
                    ...
                    // Compute current week’s Monday (start) and Sunday (end)
                    ...
                    const currentDay = format(today, "EEE").toLowerCase(); // e.g. mon, tue
                    ...
                    for (const docSnap of snapshot.docs) {
                        ...
                        // --- Handle recurrence start/end ---
                        if (recurrence.startDate) startDate = parseISO(recurrence.startDate);
                        if (recurrence.endDate) endDate = parseISO(recurrence.endDate);

                        // --- Handle once events ---
                        if (freq === "once" && recurrence.startDate) {
                            startDate = parseISO(recurrence.startDate);
                            endDate = recurrence.endDate ? parseISO(recurrence.endDate) : startDate;
                        }

                        if (!startDate) continue;

                        // ✅ Weekly recurring event check
                        let showEvent = false;
                        if (freq === "weekly") {
                            const isInDateRange = isWithinInterval(today, { start: startDate, end: endDate || today });
                            const occursThisWeek = recurrence.daysOfWeek?.includes(currentDay);
                            if (isInDateRange && occursThisWeek) showEvent = true;
                        }

                        // ✅ Once event check (must happen this week)
                        if (freq === "once") {
                            const isInThisWeek =
                                isWithinInterval(startDate, { start: monday, end: sunday }) ||
                                (endDate && isWithinInterval(endDate, { start: monday, end: sunday }));
                            if (isInThisWeek) showEvent = true;
                        }
