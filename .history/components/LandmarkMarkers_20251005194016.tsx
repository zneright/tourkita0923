        ...
        const fetchData = async () => {
            try {
                ...
                if (selectedCategory === "Events") {
                    ...
                    // Get the current week's Monday (00:00) and Sunday (23:59)
                    ...
                    for (const docSnap of snapshot.docs) {
                        ...
                        // ✅ Get event start and end (support both recurrence and direct fields)
                        ...
                        } else if (data.recurrence?.startDate) {
                            startDate = parseISO(data.recurrence.startDate);
                        }

                        if (data.dateEnd) {
                            endDate =
                                typeof data.dateEnd === "string"
                                    ? parseISO(data.dateEnd)
                                    : data.dateEnd.toDate();
                        } else if (data.recurrence?.endDate) {
                            endDate = parseISO(data.recurrence.endDate);
                        }

                        // ✅ Skip if we can't read any date
                        if (!startDate && !endDate) continue;

                        // ✅ Check if event is within the current week
                        const isInThisWeek =
                            isWithinInterval(startDate ?? today, { start: monday, end: sunday }) ||
                            (endDate && isWithinInterval(endDate, { start: monday, end: sunday })) ||
                            (startDate && endDate && startDate <= sunday && endDate >= monday);
