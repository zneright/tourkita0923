...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
            ...
            try {
                ...
                if (selectedCategory === "Events") {
                    ...
                    for (const docSnap of snapshot.docs) {
                        ...
                        const startDate = recurrence.startDate ? parseISO(recurrence.startDate) : parseISO(data.startDate);
                        const endDate = recurrence.endDate ? parseISO(recurrence.endDate) : parseISO(data.endDate);

                        const isInDateRange =
                            startDate && endDate ? isWithinInterval(todayDate, { start: startDate, end: endDate }) : true;

                        // Filter by current month
                        if (startDate.getMonth() !== currentMonth || startDate.getFullYear() !== currentYear) continue;

                        // Determine if event happens today
                        ...
                        if (!occursToday && recurrence.frequency === "weekly") continue;
    ...
    const points = filtered.map((landmark, index) =>
        point([landmark.longitude, landmark.latitude], {
            ...
            isToday: landmark.occursToday || false,
        })
    );
