            ...
            try {
                ...
                if (selectedCategory === "Events") {
                    ...
                    for (const docSnap of snapshot.docs) {
                        ...
                        // Default to main start/end if recurrence ones are empty
                        ...
                        // ✅ ONCE EVENTS: Show if today <= event start date (upcoming) or it's ongoing today
                        if (freq === "once") {
                            if (
                                startDate >= today || // upcoming
                                (endDate && isWithinInterval(today, { start: startDate, end: endDate })) // ongoing
                            ) {
                                showEvent = true;
                            }
                        }

                        // ✅ WEEKLY EVENTS: Show if today is within the recurrence date range AND matches today’s weekday
                        if (freq === "weekly") {
                            const isInDateRange =
                                isWithinInterval(today, { start: startDate, end: endDate || today });
                            const occursToday = recurrence.daysOfWeek?.includes(currentDay);
                            if (isInDateRange && occursToday) showEvent = true;
                        }
                        ...
                        // Auto-fetch lat/lng from linked marker if missing
